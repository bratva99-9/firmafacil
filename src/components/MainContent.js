    ...
    <div style={{ background: '#f8fafc', borderRadius: 18, padding: 18, boxShadow: '0 12px 30px rgba(148,163,184,0.35)', border: '1px solid rgba(148,163,184,0.3)', color: '#1f2937', fontFamily: '"JetBrains Mono", "Fira Code", monospace' }}>
      ...
      {autenticado && !abierta && (
        <div className="tools-grid">
          ...
          <div className="tool-card" onClick={() => setAbierta('estado-tributario-pagado')}>
            <div className="tool-icon">💰</div>
...
function EstadoTributarioPagadoTool() {
  const [ruc, setRuc] = useState('')
  const [cargando, setCargando] = useState(false)
  const [error, setError] = useState('')
  const [resultado, setResultado] = useState(null)
  const [pagoInfo, setPagoInfo] = useState(null) // Información del pago de Cryptomus
  const [pagoCompletado, setPagoCompletado] = useState(false) // Si el pago está completado
  const [creandoPago, setCreandoPago] = useState(false) // Si está creando el pago
  const pdfRef = useRef(null)

  // Función para crear un pago con Cryptomus
  const crearPago = async () => {
    ...
    try {
      // Generar un order_id único basado en RUC y timestamp
      const orderId = `ET_${r}_${Date.now()}`
      
      // Calcular el monto (por ejemplo, $5 USD por obligación, mínimo $10)
      const montoBase = 10 // Monto mínimo en USD
      ...
      if (data.payment?.result?.url) {
        window.open(data.payment.result.url, '_blank')
      }
    } catch (e) {
      console.error('Error invocando cryptomus-payment:', e)
  ...
  const verificarPago = async () => {
    ...
    try {
      ...
      const estadoPago = data.payment?.result?.payment_status
      if (estadoPago === 'paid' || estadoPago === 'paid_over') {
        setPagoCompletado(true)
        // Una vez pagado, ejecutar la consulta automáticamente
        ejecutarConsulta()
      } else {
  ...
  // Función para ejecutar la consulta del estado tributario
  const ejecutarConsulta = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('Error estado-tributario:', error)
        setError(error.message || 'Error en la consulta de estado tributario pagado')
        return
      }

      if (!data?.success) {
        setError(data?.error || 'No se pudo obtener el estado tributario pagado')
        setResultado(data || null)
        return
      }
      ...
      console.error('Error invocando estado-tributario:', e)
      setError(e.message || 'Error en la consulta de estado tributario pagado')
    } finally {
      setCargando(false)
    }
  }
  ...
  const consultar = async (e) => {
    ...
    // Si no hay pago completado, mostrar error
    if (!pagoCompletado) {
      setError('Debes completar el pago antes de consultar el estado tributario')
      return
    }
  ...
  const descargarPDF = async () => {
    ...
    try {
      ...
      const nombreArchivo = `Estado_Tributario_Pagado_${ruc || 'contribuyente'}_${new Date().toISOString().slice(0,10)}.pdf`
      pdf.save(nombreArchivo)
    } catch (e) {
      console.error('Error al generar PDF de estado tributario:', e)
      alert('No se pudo generar el PDF. Intenta nuevamente.')
  ...
  return (
    <div className="tool-panel" style={{ padding: 12 }}>
      ...
      <h3 style={{ fontSize: 16, fontWeight: 800, margin: '0 0 6px' }}>Estado tributario pagado (SRI)</h3>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        <p style={{ fontSize: 12, color: '#6b7280', margin: 0 }}>
          Ejecuta el flujo automático (consulta código de persona + captcha + token SRI + estado tributario) usando solo el RUC. Requiere pago previo.
        </p>
        {resultado?.estadoTributario && (
          <button
            type="button"
        ...
        <div style={{ 
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <p style={{ fontSize: '12px', color: '#78350f', margin: '4px 0 0 0' }}>
              Para acceder a esta consulta, primero debes completar el pago mediante criptomonedas.
            </p>
          </div>
          
          <form className="et-form" style={{ marginBottom: '8px' }}>
            <input
              className="et-input"
              value={ruc}
              onChange={(e) => setRuc(e.target.value)}
              ...
              <div style={{ marginBottom: '35px', borderBottom: '2px solid #1a1a1a', paddingBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#666666' }}>
                      Ref: Estado Tributario SRI Pagado
                    </div>
                  </div>
                </div>
              </div>
